...
|  | - Web 控制台 (端口 34115) | <--(REST)-- | - WireGuard 虚拟 IP (100.64.0.x)   |  |
...
前端静态资源由客户端内置 HTTP 服务托管（默认监听 `127.0.0.1:34115`），前端不持有公网控制端的长效特权密钥，仅通过本地 REST 接口与后端网络核心交互：

1. **网络状态指示横栏 (System Ribbon)**：
   - 虚拟 IP：通过 `#val-virtual-ip` 呈现（初始值 `100.64.0.5`）。
   - 用户态协议栈状态：通过 `#val-tsnet-state` 显示连通状态。
   - 本地研发代理：通过 `#val-socks5-state` 显示 `127.0.0.1:1055` 运行状态。
   - 熔断基线指标：固定展示 `T_timeout <= 3000ms`。
2. **拓扑与服务交互接口**：
   - `GET /api/status`：拉取系统整体指标与用户态节点状态。
   - `GET /api/config`：读取已脱敏的当前运行配置，支撑用户自查面板。
   - `GET /api/peers`：拉取同网段成员节点列表（包含虚拟 IP、显示名、在线状态与 P2P/DERP 徽标）。
   - `GET /api/dev-servers`：拉取受保护的内部开发机列表（`100.64.0.10+`，`tag:server`）。
...
| **前端自查面板** | 浏览器打开 `http://127.0.0.1:34115` 访问“用户配置自查” | 虚拟 IP 正确呈现，媒体服务显示“服务端物理隔离” | 检查 `/api/config` 接口及客户端本地 `config.json` 是否存在 |
